...
def inventory_restore_from_payload(data, send_progress=None):
    ...
    backup_path = None
    backup_content = None
    backup_path_status = None
    backup_content_status = None

    def progress(step, message, status=None):
    ...
    try:
        # 1. Backup current files (stock.json and stock_status.json)
        progress("backup", "Backing up current stock.json and stock_status.json…")
        backup_content = keyme.config.load(STOCK_FILENAME, logging=False)
        if backup_content is None:
            backup_content = {}
        backup_path = STOCK_FILENAME + ".restore_backup." + timestamp_suffix
        keyme.config.save(backup_path, backup_content, sorted=True)
        backup_content_status = keyme.config.load(STOCK_STATUS, logging=False)
        if backup_content_status is None:
            backup_content_status = {}
        backup_path_status = STOCK_STATUS + ".restore_backup." + timestamp_suffix
        keyme.config.save(backup_path_status, backup_content_status, sorted=False)
        progress("backup", "Backup complete.", "ok")

        # 2. Stop INVENTORY
        progress("stop_inventory", "Stopping INVENTORY process…")
        keyme.ipc.send_sync("MANAGER", "STOP_PROCESS", {"process": "INVENTORY"})
        time.sleep(10)
        progress("stop_inventory", "INVENTORY stopped.", "ok")

        # 3. Write new stock
        progress("write_stock", "Writing new stock…")
        keyme.config.save(STOCK_FILENAME, stock, sorted=True)
        progress("write_stock", "Stock file written.", "ok")

        # 4. Start INVENTORY
        progress("start_inventory", "Starting INVENTORY process…")
        keyme.ipc.send_sync("MANAGER", "START_PROCESS", {"process": "INVENTORY"})
        time.sleep(20)
        progress("start_inventory", "INVENTORY started.", "ok")

        # 5. Verify (get_magazine_list + update_pricing)
        progress("verify", "Verifying inventory…")
        interface = _inventory_interface()
        try:
            stock_list = interface.get_magazine_list(include_status_data=True)
        except Exception as e:
            keyme.log.warning(f"inventory_restore_from_payload: verify failed: {e}")
            progress("verify", "Verification failed.", "failed")
            raise
        if not stock_list or not isinstance(stock_list, dict):
            progress("verify", "Verification failed.", "failed")
            raise ValueError("get_magazine_list returned invalid data")
        if getattr(keyme.config, "IS_KIOSK", False):
            progress("verify", "Running update_pricing…")
            try:
                from util.update_pricing import update_pricing
                if update_pricing() != 0:
                    progress("verify", "Update pricing failed.", "failed")
                    raise ValueError("update_pricing returned non-zero")
            except Exception as e:
                keyme.log.warning(f"inventory_restore_from_payload: update_pricing failed: {e}")
                progress("verify", "Verification failed (update_pricing).", "failed")
                raise
        progress("verify", "Verification passed.", "ok")

        progress("done", "Restore complete.", "ok")
        return WebsocketSuccess({}).to_json()
        ...
        try:
            if backup_path and backup_content is not None:
                keyme.config.save(STOCK_FILENAME, backup_content, sorted=True)
            if backup_path_status is not None and backup_content_status is not None:
                keyme.config.save(STOCK_STATUS, backup_content_status, sorted=False)
