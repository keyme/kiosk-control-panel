...
export default function BittingCalibrationImagesPage({ kioskName: kioskNameProp }) {
  const { date } = useParams();
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const kioskName = searchParams.get('kiosk_name') || kioskNameProp;
  const bittingPath = kioskName ? `/${kioskName}/calibration/report/bitting_calibration` : '/calibration/report/bitting_calibration';
    ...
    return (
      <Card>
        <CardContent className="pt-6">
          ...
          <button
            type="button"
            onClick={() => navigate(bittingPath)}
            className="mt-2 text-sm underline"
          >
            Back to Bitting Calibration
          </button>
        </CardContent>
      </Card>
    );
  ...
  if (!sections || Object.keys(sections).length === 0) {
    return (
      <Card>
        <CardContent className="pt-6">
          <p className="text-muted-foreground text-sm">No images for this date.</p>
          <button
            type="button"
            onClick={() => navigate(bittingPath)}
            className="mt-2 text-sm underline"
    ...
    <>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <span className="text-muted-foreground text-sm">Date: {formatDateDisplay(date)}</span>
        </div>
