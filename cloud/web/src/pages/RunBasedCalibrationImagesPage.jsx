...
export default function RunBasedCalibrationImagesPage({ sectionId, kioskName: kioskNameProp }) {
  ...
  const reportPath = kioskName ? `/${kioskName}/calibration/report/${sectionId}` : `/calibration/report/${sectionId}`;
  ...
  const sectionNames = sections
    ? Object.keys(sections).filter((k) => k !== 'run_start_ts' && k !== 'run_end_ts')
    : [];
  if (!sections || sectionNames.length === 0) {
    return (
      <Card>
        <CardContent className="pt-6">
          <p className="text-muted-foreground text-sm">No images for this run.</p>
          <button
            type="button"
            onClick={() => navigate(reportPath)}
            className="mt-2 text-sm underline"
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <span className="text-muted-foreground text-sm">
            Run: {sections.run_start_ts && sections.run_end_ts
              ? formatRunSpan(sections.run_start_ts, sections.run_end_ts)
              : formatRunIdForDisplay(runId)}
          </span>
        </div>
        {sectionNames.map((sectionName) => (
          <Card key={sectionName}>
            <CardHeader>
              <CardTitle className="text-base">{sectionTitle(sectionName)}</CardTitle>
            </CardHeader>
